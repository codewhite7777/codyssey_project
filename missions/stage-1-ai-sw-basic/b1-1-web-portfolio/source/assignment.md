# [원문] B1-1 나를 소개하는 웹페이지 처음부터 만들기

> 전달받은 과제 원문. **수정 금지.** 해석·요약은 `../MISSION_SPEC.md`에서 한다.
> 접수일: 2026-09-17 / 전달 경로: 사용자 직접 전달(텍스트)
...
- **프로젝트 필터링**: GitHub 프로젝트를 언어별로 필터링하는 버튼을 만든다. `array.filter()` 활용
- **타이핑 효과**: Hero 섹션에 타자기처럼 한 글자씩 나타나는 효과를 구현한다.
- **폼 실제 전송**: Formspree 또는 EmailJS를 연동하여 실제 이메일을 전송한다.
- **시스템 다크 모드 감지**: `prefers-color-scheme` 미디어 쿼리로 시스템 설정을 감지한다.
