# 나를 소개하는 웹페이지 처음부터 만들기 — 구현 명세

| 항목 | 값 |
|---|---|
| 폴더 슬러그 | `web-portfolio` (불변) |
...
| R-ID | 요구사항 | 구현 위치 | V | C | E |
|---|---|---|---|---|---|
| R-14 | 외부 스타일시트 `css/style.css` 사용 | | V-1 | — | — |
...
| R-22 | 버튼·카드 hover 효과 + `transition` 적용 | | V-9 | — | — |
| R-23 | 카드에 `box-shadow` 적용 | | V-9 | — | — |
...
| R-28 | `textContent` / `innerHTML`로 내용 변경 | | V-23 | C-3 | E-4.2 |
...
| R-33 | nav 메뉴 클릭 시 해당 섹션으로 **부드러운 스크롤** | | V-12 | — | E-1.2 |
| R-34 | 스크롤 **300px** 이상에서 탑 버튼 표시, 클릭 시 최상단 이동 (임계값 README 명시) | | V-13 | — | E-1.2 |
| R-35 | 스크롤 **60px** 이상에서 nav 배경색 변경 (임계값 README 명시) | | V-14 | — | E-1.2 |
...
| R-39 | Contact 폼 3필드 존재 (이름 / 이메일 / 메시지) | | V-17 | — | E-1.5 |
| R-40 | 필수값 검증 — 빈 필드 제출 불가 | | V-17 | C-6 | E-2.5 |
| R-41 | 이메일 형식 검증 | | V-17 | C-6 | E-2.5 |
| R-42 | 에러 메시지가 **해당 입력 필드 근처**에 표시 | | V-17 | C-6 | E-2.5 |
| R-43 | 제출 시 `preventDefault()` + 성공 메시지 표시 | | V-17 | C-3, C-6 | E-3.2 |
...
| R-45 | 템플릿 리터럴로 HTML 동적 생성 | | V-23 | C-4 | E-3.3, E-4.2 |
| R-46 | 구조분해 할당으로 객체/배열 값 추출 | | V-23 | C-4 | E-3.3 |
| R-47 | `map`으로 GitHub 데이터 → HTML 카드 변환, `forEach`로 순회 | | V-19 | C-4 | E-3.3 |
...
| R-50 | **성공 상태** UI (카드 리스트 렌더링) | | V-19 | C-5, C-6 | E-2.4 |
...
| R-ID | 요구사항 | 구현 위치 | V | C | E |
|---|---|---|---|---|---|
| R-56 | GitHub Pages 배포 + 외부 접속 가능한 URL 존재 | | V-24 | — | E-1.6 |
| R-57 | 배포 URL에서 반응형·인터랙션·API·폼 검증이 모두 동작 | | V-24 | — | E-1.6 |
| R-58 | README에 설명·사용 기술·배포 URL·스크린샷 + **임계값 3개(300/60/0.2)** 명시 | | V-25 | — | E-1.6 |
...
C-7 ~ C-10은 요구사항을 구현하면 필연적으로 마주치는 개념이라 추가했다.

| C-ID | 개념 | 왜 이 과제에 필요한가 | 노트 |
|---|---|---|---|
